...
class PatientListingRetrievalModel(tfrs.models.Model):

    def __init__(self, unique_patient_ids, unique_listing_ids, listings):
        super().__init__()
        ...
        self.candidate_model = tf.keras.Sequential([
            ListingNetwork(unique_listing_ids),
            tf.keras.layers.Dense(32)
        ])

        self.task = tfrs.tasks.Retrieval(
            metrics=tfrs.metrics.FactorizedTopK(
                candidates=listings.batch(128).map(self.candidate_model),
            ),
        )
